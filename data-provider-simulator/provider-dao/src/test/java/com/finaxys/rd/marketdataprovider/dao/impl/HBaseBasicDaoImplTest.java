...
import com.finaxys.finaxysplatform.dataprovidersimulator.dao.exception.HBaseIOException;
...
//To move to the finaxys-pmatform-persistence
...
public abstract class HBaseBasicDaoImplTest<T> {
	...
	public final void test_executeAdd() throws IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		...
		// arrays contents.
		//!!!!!!!!!!!!!!!!!!!!!!!! we sould use the type manager 
		//assertTrue(Arrays.equals(put.getRow(),
		// HBaseUtils.makeRowKey(fixtureBean).getKey()));
		...
		for (Field attribute : attributes)
			if (!attribute.getName().contains("serialVersionUID") && (keyValue = put.get(target.DEFAULT_COLUMN_FAMILY, attribute.getName().getBytes())).size() > 0)
				assertTrue(Arrays.equals(keyValue.get(0).getValue(), DaoHelper.toBytes(attribute, PropertyUtils.getSimpleProperty(fixtureBean, attribute.getName()))));
...
//weshoul use the new type manager		assertTrue(Arrays.equals(HBaseUtils.makeRowKey(list.get(0)).getKey(), HBaseUtils.makeRowKey(getFixtureBean()).getKey()));
	}
